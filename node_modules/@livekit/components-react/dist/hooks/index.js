...
function useRemoteParticipant(identity, options = {}) {
  const room = useRoomContext();
  const [updateOnlyOn] = React29.useState(options.updateOnlyOn);
...
function useRoomInfo(options = {}) {
  const room = useEnsureRoom(options.room);
  const infoObserver = React30.useMemo(() => (0, import_components_core23.roomInfoObserver)(room), [room]);
...
function useSpeakingParticipants() {
  const room = useRoomContext();
  const speakerObserver = React31.useMemo(() => (0, import_components_core24.activeSpeakerObserver)(room), [room]);
...
function useSortedParticipants(participants) {
  const [sortedParticipants, setSortedParticipants] = React32.useState(
...
function useStartAudio({ room, props }) {
  const roomEnsured = useEnsureRoom(room);
  const { className, roomAudioPlaybackAllowedObservable: roomAudioPlaybackAllowedObservable2, handleStartAudioPlayback } = React33.useMemo(
...
function useStartVideo({ room, props }) {
  const roomEnsured = useEnsureRoom(room);
  const { className, roomVideoPlaybackAllowedObservable, handleStartVideoPlayback } = React34.useMemo(
...
function useSwipe(element, options = {}) {
  var _a;
  const touchStart = React35.useRef(null);
  const touchEnd = React35.useRef(null);
  ...
  const onTouchEnd = React35.useCallback(() => {
...
function useChatToggle({ props }) {
  ...
  const { className } = React36.useMemo(() => (0, import_components_core28.setupChatToggle)(), []);
...
function useToken(tokenEndpoint, roomName, options = {}) {
  const [token, setToken] = React37.useState(void 0);
  React37.useEffect(() => {
    var _a;
...
function useTrackMutedIndicator(trackRef) {
  var _a, _b;
  const trackReference = useEnsureTrackRef(trackRef);
  const { className, mediaMutedObserver } = React38.useMemo(
...
function useTrackToggle(_a) {
  ...
  const userInteractionRef = React39.useRef(false);
  const { toggle, className, pendingObserver, enabledObserver } = React39.useMemo(
  ...
  const newProps = React39.useMemo(() => mergeProps(rest, { className }), [rest, className]);
  const clickHandler = React39.useCallback(
    (evt) => {
      var _a3;
...
function useTracks(sources = [
  ...
  const room = useEnsureRoom(options.room);
  const [trackReferences, setTrackReferences] = React40.useState([]);
  const [participants, setParticipants] = React40.useState([]);
  const sources_ = React40.useMemo(() => {
  ...
  const maybeTrackReferences = React40.useMemo(() => {
...
function useTrackRefBySourceOrName(source) {
  var _a, _b;
  const [publication, setPublication] = React41.useState((0, import_components_core33.getTrackByIdentifier)(source));
  const { trackObserver } = React41.useMemo(() => {
...
function useIsEncrypted(participant) {
  ...
  const observer = React44.useMemo(() => (0, import_components_core36.encryptionStatusObservable)(room, p), [room, p]);
...
var useTrackVolume = (trackOrTrackReference, options = { fftSize: 32, smoothingTimeConstant: 0 }) => {
  ...
  const [volume, setVolume] = React45.useState(0);
  React45.useEffect(() => {
...
var useMultibandTrackVolume = (trackOrTrackReference, options = {}) => {
  ...
  const [frequencyBands, setFrequencyBands] = React45.useState([]);
...
function useParticipantTracks(sources, participantIdentity) {
  ...
  const observable = React46.useMemo(
