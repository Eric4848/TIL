...
function useRoomInfo(options = {}) {
  const room = useEnsureRoom(options.room);
  const infoObserver = React30.useMemo(() => roomInfoObserver(room), [room]);
...
function useSpeakingParticipants() {
  const room = useRoomContext();
  const speakerObserver = React31.useMemo(() => activeSpeakerObserver(room), [room]);
...
function useChatToggle({ props }) {
  ...
  const { className } = React36.useMemo(() => setupChatToggle(), []);
...
function useTrackRefBySourceOrName(source) {
  var _a, _b;
  const [publication, setPublication] = React41.useState(getTrackByIdentifier(source));
...
function useIsEncrypted(participant) {
  ...
  const observer = React44.useMemo(() => encryptionStatusObservable(room, p), [room, p]);
